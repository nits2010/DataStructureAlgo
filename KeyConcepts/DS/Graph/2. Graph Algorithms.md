...
| **Bellman-Ford**   | Negative edge weights./ Include K edges only             | $O(V \times E)$ | Slower; identifies negative cycles.                      |
...
| Algorithm            | Best Use Case                                                  | Time Complexity          | Key Constraint                                    | Secret Sauce (The "How")                                                                                               |
| -------------------- | ------------------------------------                           | ----------------------   | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Kruskal’s (MST)**  | Connect all nodes for minimum cost.                            | $O(E \log E)$            | Sorting edges makes it better for sparse graphs.  | **Greedy + Sorting:** Sort all edges by weight and pick the smallest ones that don't cause a cycle.                    |
| **Prim’s (MST)**     | Connect all nodes for minimum cost.                            | $O(E \log V)$ or O (V^2) | Better for dense graphs (many edges).             | **Greedy + BFS-like:** Start at a node and grow the tree by picking the cheapest edge connecting to an unvisited node. With Matrix its O(v^2) |
| **Kahn’s (BFS)**     | Topological Sort / Dependency order.                           | $O(V + E)$               | Only works on **DAGs** (Directed Acyclic Graphs). | **In-degree Tracking:** Keep a count of incoming edges; only process nodes with an in-degree of 0.                     |
| **Tarjan’s**(SCC)    | Finding strongly connected clusters.                           | $O(V + E)$               | For Directed Graphs only.                         | **Low-link Values:** A single DFS pass that identifies nodes that can reach their own ancestors.                       |
| **Kosaraju’s**(SCC)  | Finding strongly connected clusters.                           | $O(V + E)$               | For Directed Graphs only.                         | **Double DFS + Reverse:** DFS to order nodes, flip the arrows, then DFS again to harvest clusters.                     |
| **Union-Find (DSU)** | Merging sets / Cycle detection / Dynamic connectivity.         | $O(E \cdot \alpha(V))$   | Extremely fast for "dynamic" connectivity.        | **Path Compression:** "Flatten" the tree structures so checking if two nodes are in the same set is nearly $O(1)$.     |
...
- **No (Exploration):** Use **BFS** (unweighted) or **Dijkstra** (weighted) to map out everything or  **Bellman Ford** if negative weights or k edges query.
...
- [Next Chapter](https://github.com/nits2010/DataStructureAlgo/blob/preparation-2025/KeyConcepts/DS/Graph/3.%20mental%20model.md)
