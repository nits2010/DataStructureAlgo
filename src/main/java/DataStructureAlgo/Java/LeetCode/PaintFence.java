...
public class PaintFence {
     ...
     * * If we paint the first fence with color = K1
     * * then we can paint the second fence with different color = {K2,...Kk} i.e. we left with k-1 possibility in this case.
     ...
     * * Since we can't have same color for three different fence in a row i.e. we can't choose 'same' from two fence. But we can choose 'different' from two
     * fence, because 'different' has different color of
     * last two fence. Since we need to paint last fence (3rd one) with same color
     * as second last fence, then we left with only 1 choice.
     * <p>
     * Hence Same = different {choices when we paint last two fence with different
     * color} * 1{for this fence, since we need same color as last fence}
     * <p>
     * Case 1: Both last two fence has different color.
     * * Since last two fence has different color i.e. we can put k-1 choices {since
     * we need to make last fence has different color than second last fence}.
     * * Which means either last two fence has same color and this one has different
     * color Or last two has different color and this one also have different color
     * <p>
     * Hence different = (same + different) * (k-1)
     * <p>
     * Hence for 3 fence the answer is Same + different = (k*(k-1))*1 + (k*1 +
     * k*(k-1)) * (k-1) = 2*k*(k-1) + k*(k-1)^2
     *
     * @param n
     * @param k
     * @return
    ...
    public static int numWays(int n, int k) {
        ...
        int same = k; // Same = k {k choices for first fence} * 1 {one choice for second to be same as
                      // first}
        int different = k * (k - 1); // different = k {k choices for first fence} * k-1 {k-1 choice for second to be
        ...
        for (int i = 3; i <= n; i++) {
            ...
            same = prevDiff; // Same = different {choices when we paint last two fence with different color}
                             // * 1{for this fence, since we need same color as last fence}
        }
