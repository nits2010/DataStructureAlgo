...
public class WallsAndGates_286 {
    ...
    /**
     * Intuitions:
     * To find the gate from an empty room, we need to try all the four directions
     * possible and find the nearest gate. It is worth to note that, if an empty
     * room
     * already found a gate with minimum distance X and if there is another empty
     * room near to it, then the distance of this gate can be calculate from already
     * computed
     * empty room distance.
     * <p>
     * Approach:
     * 1. Start traversing the entire grid, if we find a "gate", then we try all the
     * four directions and find the nearest empty room.
     * 2. As soon as we go in any valid direction, then the distance to the nearest
     * empty room will be increased by 1.
     * 3. As soon as we find an empty room, we update the distance to the gate with
     ...
     * <p>
     * Approach:
     * To apply BFS, we need to find all the gates and then search for nearest empty
     * room and update the distance.
     * 1. Enqueue all the gates.
     * 2. Start search for a given gate and as soon we hit the empty room, update
     * the distance.
        ...
        public void wallsAndGates(int[][] rooms) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (int k = 0; k < row.length; k++) {
                    ...
                    // if a neighbor is a wall, then avoid it as well, we should not touch the other
                    // gate
                    // hence consider only empty room
                    if (isSafe(neighRow, neighCol, m, n) && rooms[neighRow][neighCol] == INF) {

                        // we found an empty room [neighRow][neighCol] with distance d of the current
                        // gate
                        rooms[neighRow][neighCol] = rooms[gateRow][gateCol] + 1; // it will at least 1 distance far from
                                                                                 // a gate

                        // check other neighbors, now this neighbor is a path to an empty gate for other
                        // neighbors
