 ...
 * , https://leetcode.ca/all/305.html#google_vignette
 * Descriptions
 * <p>
 ...
 * Explanation:
 * <p>
 * Initially, the 2d grid is filled with water. (Assume 0 represents water and 1
 * represents land).
 * <p>
 * 0 0 0
 * 0 0 0
...
public class NumberOfIslandII_305 {
    public static void main(String[] args) {
        ...
        tests.add(test(3, 3, new int[][] { { 0, 1 }, { 0, 1 }, { 2, 1 }, { 2, 2 } }, List.of(1, 1, 2, 2))); // Duplicate
                                                                                                            // land
                                                                                                            // entry
        tests.add(test(4, 4, new int[][] { { 0, 1 }, { 1, 1 }, { 2, 1 }, { 2, 2 }, { 3, 3 } }, List.of(1, 1, 1, 1, 2)));
        tests.add(test(5, 4, new int[][] { { 0, 1 }, { 1, 1 }, { 2, 1 }, { 2, 2 }, { 3, 3 }, { 4, 3 } },
                List.of(1, 1, 1, 1, 2, 2)));
    ...
    private static boolean test(int m, int n, int[][] positions, List<Integer> expected) {
        ...
        finalPass &= pass;
        CommonMethods.printTest(new String[] { "SolutionUsingIslandFinder", "Pass" }, false, output,
                pass ? "PASS" : "FAIL");

        output = new SolutionUsingUnionFind().numIslands2(m, n, positions);
        pass = CommonMethods.compareResultOutCome(output, expected, true);
        finalPass &= pass;
        CommonMethods.printTest(new String[] { "SolutionUsingUnionFind", "Pass" }, false, output,
                pass ? "PASS" : "FAIL");

        output = new SolutionUsingCachingMap().numIslands2(m, n, positions);
        pass = CommonMethods.compareResultOutCome(output, expected, true);
        finalPass &= pass;
        CommonMethods.printTest(new String[] { "SolutionUsingCachingMap", "Pass" }, false, output,
                pass ? "PASS" : "FAIL");
    ...
    /**
     * As post of the add operation, the problem reduces to find the number of
     * islands in current matrix; we can use the
     * island finder as subroutine {@link NumberOfIsland_200}.
     * For each operation, the time complexity would be O(mn), where m and n are the
    ...
    /**
     * Above algorithm first build the grid, so land, then count the island. There
     * are two important things
     * 1. The reason is that for each position either creates a new island
     * or
     * 2. Get connected to old island.
     * <p>
     * Which turns out, if we somehow efficiently can find that two islands are
     * connected or not, then we can simply discard the new land as a potential new
     * island.
     * Since, an island can be connected through 4 directions {horizontal and
     * vertical} i.e., means we need to attach those valid position to old island as
     * well to correctly evaluate
     * the new position as potential island.
     * <p>
     * Union-Find {@link UnionFindDisjointSets}
     * -----------
     * Using union-Find we can find out that two points are connected or not by
     * checking their parent.
     * <p>
     * <p>
     * Algorithm:
     * 1. Create the possible amount of space available as empty parent {our water
     * spaces}
     * 2. For each position, evaluate the following things
     * * 2.1 : does this land be being connected to any previous island ? if so,
     * then we should not count this as an island.
    ...
    static class SolutionUsingUnionFind {
        ...
        private boolean addLand(int[] pos, int m, int n, int[][] island, UnionFind unionFind) {
            ...
            if (island[pos[0]][pos[1]] == 1) {
                ...
                // check its neighbors, if this is an island, than there will be only water
                // around it. otherwise connect all lands around it.
                for (int k = 0; k < row.length; k++) {
                    int i = pos[0] + row[k];
                    int j = pos[1] + col[k];
                    int neighborIndex = _2DPos1DPost(i, j, m, n);

                    // if neighbor is land, then this must have been evaluated in last addLandCall,
                    // just union them make in single land with current land
                    if (isSafe(i, j, m, n) && island[i][j] == 1) {
                        isNewConnectedIslandFound = false; // it must be connected in last add call, hence must be
                                                           // calculated
        ...
        private int _2DPos1DPost(int i, int j, int m, int n) {
            ...
            // if 'i' represent m, then i*n represent the position in 2D of row and
            // addition, j makes column position
            return i * n + j;
        }
