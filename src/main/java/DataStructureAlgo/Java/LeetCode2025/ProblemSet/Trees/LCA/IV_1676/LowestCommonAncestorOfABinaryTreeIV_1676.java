 ...
 * Author: Nitin Gupta
 * Date:17/08/24
 * Question Title: 1676. Lowest Common Ancestor of a Binary Tree IV
 * Link:
 ...
 * @Amazon
 * @Editorial <a href=
 *            "https://leetcode.ca/2020-07-02-1676-Lowest-Common-Ancestor-of-a-Binary-Tree-IV">...</a>
 */
...
public class LowestCommonAncestorOfABinaryTreeIV_1676 {
    ...
    public static void main(String[] args) {
        ...
        test &= test(new Integer[] { 3, 5, 1, 6, 2, 0, 8, null, null, 7, 4 },
                new Integer[] { 0, 1, 2, 3, 4, 5, 6, 7, 8 }, 3);
        test &= test(new Integer[] { 3, 5, 1, 6, 2, 0, 8, null, null, 7, 4 },
                new Integer[] { 0, 1, 2, 3, 4, 5, 6, 7, 18 }, null);
    ...
    private static boolean test(Integer[] tree, Integer[] nodeElements, Integer expected) {
        System.out.println("-----------------------");
        System.out.println("Input :" + Arrays.toString(tree) + " Nodes : " + Arrays.toString(nodeElements)
                + " Expected : " + expected);
        ...
        System.out.println("LCA Using Recursive App : " + lcaUsingRecursive + " Test LCA Using Recursive : "
                + (testUsingRecursive ? "Passed" : "Failed"));

        SolutionRecursiveNodeDoesNotExist solutionRecursiveNodeDoesNotExist = new SolutionRecursiveNodeDoesNotExist();
        TreeNode lcaUsingRecursiveNodeDoesNotExist = solutionRecursiveNodeDoesNotExist.lowestCommonAncestor(root,
                nodes);
        boolean testUsingRecursiveNodeDoesNotExist = lcaUsingRecursiveNodeDoesNotExist == expectedNode;
        System.out.println("LCA Using Recursive App Node does not exits : " + lcaUsingRecursiveNodeDoesNotExist
                + " Test LCA Using Recursive Node does not exits : "
                + (testUsingRecursiveNodeDoesNotExist ? "Passed" : "Failed"));
        ...
        System.out.println("LCA Using Parents Map : " + lcaUsingParentsMap + " Test LCA Using Parents Map : "
                + (testLCAUsingParentsMap ? "Passed" : "Failed"));
    ...
    /**
     * This is similar, instead of checking for two nodes, here we need to check for
     * multiple nodes. That's all.
    ...
    static class SolutionRecursive {
        ...
        public TreeNode lowestCommonAncestor(TreeNode root, TreeNode[] nodes) {
            ...
            Set<TreeNode> set = Arrays.stream(nodes).collect(Collectors.toSet()); // convert nodes to set for faster
                                                                                  // search
     ...
     * 3.1.1 if yes, then check does its parent present in final LCA Set, if yes
     * then skip this node, otherwise add it to lca set.
     * 3.1.2 if no. then get its parent from parents map and repeat
     * <p>
     * 4. Post getting LCA set, we need to reduce it to one node. Start searching
     * each lca node and search its parent in lca set.
