        ...
        public boolean primeSubOperation(int[] nums) {
            ...
            for (int i = nums.length - 2; i >= 0; i--) {
                ...
                if (nums[i] >= nums[i + 1]) {
                    ...
                    // we need to fix nums[i], to fix it, we need a prime number that is less than
                    // nums[i] but greater than nums[i] - nums[i+1]
                    // this is because to make nums[i] < nums[i+1] => nums[i] - nums[i+1] < 0, hence
                    // if prime < nums[i] and prime > nums[i] - nums[i+1]
                    // then nums[i] - prime < nums[i+1]

                    // example [9,6]
                    // nums[0] = 9, nums[1] = 6
                    // nums[0] - nums[1] = 3, means if we subtract '3' from nums[0] then it will be
                    // 9-3 = 6 which makes them same [6,6] to make it increasing
                    // nums[0] < 6 hence, if we find a prime number that is greater than 3 but less
                    // than 9 than we can make it increasing, like prime= 7
                    // [9-7, 6] = [2,6]
