 ...
 * Output : 42
 */
public class MaximumSubsetNoDigitsCommon {
     ...
     * <p>
     * Brute-force
     * now clearly each array of n elements has 2^n sub-set. Build a sub-set out of given n elements and check does adding a new value would violate condition 2, to check this we need to somehow get to know what are the digits available in a our given array so far. Since we are asked to have digits which has 0 to 9 only.
     * So, what we can do is, we keep having the masking [put 1 corresponding to a digit in a[i]] of a number in a subset. So whenever a new number we try to add, we check does masking of this number is "kinda" present or not ( is there any place in our 0-9 mask which is already set and this number also trying to set)
     * <p>
     ...
     * Our main bottel neck is 2^n. As you can see, what we essentially try to do above is finding a set s.t. it has some digits present in it and next
     * element is added only if room available.
     ...
     * if array number is also have same digits present in its binary represent then they colide essentially.
     * <p>
     * Hence
     ...
     * simple if we a "|" b == a them and it returns set 'a' [ in our case "01010"] then b = "00010" is sub-set of 'a'
     * <p>
